...
class Pipeline:
    ...
    def get_dicoms(self):
        ...
        '''
        series_list = [series for series in os.listdir(self.path) if os.path.isdir(os.path.join(self.path, series)) and 'wsx' not in series]
    ...
    def get_dicom_info(self):
        ...
        self.series_list = sorted(list(set(self.series_list).difference(set(remove_series)))) # remove any series that doesn't have minimum number of timesteps 
        ...
        if len(dicom_info) == 0:
            self.status = 'no 2D images'
            raise ValueError(f'{self.patient} has no 2D images, please check')
        else:
    ...
    def read_dicom_header(self,series):
        ...
        for dicom_path in dicoms_in_series: # go through dicom in each series
            try: # if dicom doesn't have an associate pixel array (image), ignore dicom
                ...
                try:
                    ...
                    try:
                        ...
                        image = tfio.image.decode_dicom_image(image_bytes, dtype=tf.uint16)[0]
                        image_exists = True
                    except:
                        image_exists = False                
            ...
            if image_exists: # if image exists and is not 3d read all other information
                ...
                try:
                    dicom_info[dicom_path]['seriesnumber'] = dcm.SeriesNumber
                except:
                ...
                try:
                    ...
                    dicom_info[dicom_path]['N_timesteps'] = np.nan
                try:
                    try:
    ...
    def get_stack_df_list(self, dicom_info):
        ...
        for unique_or in  unique_orientations:
            ...
            unique_df = unique_df.set_index(['pixelspacing','thickness','N_timesteps']).sort_index() # sort pixel spacing and thickness
            for unique_idx in unique_df.index.unique(): # going through all the unique orientation+pixelspacing+thickness
                ...
                if isinstance(stack_df, pd.DataFrame): # checking if there is more than one result
                    ...
                    for unique_imshape in  unique_image_shapes: # making sure all the images are of the same shape
                        stack_unique_image_shape_df = stack_df.loc[stack_df['image_shape'] == unique_imshape].reset_index()
                        if isinstance(stack_unique_image_shape_df, pd.DataFrame) and len(stack_unique_image_shape_df) > 1: 
                            separated = False
                            for uni_series in stack_unique_image_shape_df.seriesnumber.unique(): # going through each series
                                series_df = stack_unique_image_shape_df.loc[stack_unique_image_shape_df['seriesnumber'] == uni_series] 
    ...
    def get_sax_df(self,stack_df_list, sax_id_num = sax_id_num):
        '''
        ...
        '''
        for stack_id in range(len(stack_df_list)):
            stack_df_list[stack_id] = self.cine_series(stack_df_list[stack_id]) # make sure each stack has minimum number of timesteps
            stack_df_list[stack_id] = self.clean_sax_df(stack_df_list[stack_id]) # make sure each stack has minimum number of timesteps
        ...
        if len(stack_df_list) > 1:
            ...
            for stack_idx in range(len(stack_df_list)): # going through each stack
                ...
                if len(stack_df) > 0 and self.is_sax_valid(stack_df): # make sure the stack is valid
                    ...
                    if N_slices > min_slices:
                        ...
                        stack_df.loc[:,'image'] = stack_df.loc[:,'image'].map(lambda x:normalize(np.array(tf.image.resize(x[...,np.newaxis], [sax_id_image_size,sax_id_image_size], method = 'nearest'))))
            ...
            if np.max(sax_mean_probs) == 0:  # if all the stacks return 0 probability that it is a sax, there is no sax
                ...
                if len(np.where(sax_max_probs == np.max(sax_max_probs))[0]) > 1:
                    ...
                    sax_probs = sax_max_probs
                
                self.sax_probs = sax_probs
                sax_id = np.argsort(sax_probs)[::-1] # sort the stacks by probability
                valid = False
                sax_id_cnt = 0 
                while valid == False: # from the highest to lowest probability, get the first valid stack
                    sax_df = stack_df_list[sax_id[sax_id_cnt]] 
                    valid = self.is_sax_valid(sax_df)
                    sax_id_cnt += 1
                    if sax_id_cnt == len(sax_id):
                        break
                self.sax_df = sax_df.reset_index().sort_values(['slicelocation','triggertime'])
                return sax_df.reset_index().sort_values(['slicelocation','triggertime'])
        elif len(stack_df_list) ==1 and self.is_sax_valid(stack_df_list[0]): # if there is only one stack and it is valid, no need to classify
            sax_df = stack_df_list[0]
    ...
    def same_timesteps(self, sax_df):
        ...
        if sax_df.seriesnumber.nunique() > 1:
            ...
            for uni_series in sax_df.seriesnumber.unique():
                ...
                if series_df.triggertime.nunique() == N_timesteps:
                    series_df_list.append(series_df)
            if len(series_df_list) > 1:
    ...
    def remove_repeated_scans(self,sax_df):
        ...
        for uni_slice in unique_slices:
            if len(sax_df.loc[sax_df['slicelocation'] == uni_slice]) > N_timesteps:
                repeated_slices.append(uni_slice)
            else:
                if len(sax_df.loc[sax_df['slicelocation'] == uni_slice]) > min_timesteps:
                    new_sax_df_list.append(sax_df.loc[sax_df['slicelocation'] == uni_slice])
                
        if len(repeated_slices) == 0:
            return sax_df
        else:
            for rep_slice in repeated_slices: 
                repeat_sax_df = sax_df.loc[sax_df['slicelocation'] == rep_slice]
                if repeat_sax_df.seriesnumber.nunique() > 1: # if there is more than one series, take the last series
                    series_numbers = repeat_sax_df.seriesnumber.unique()
                    single_sax_df = repeat_sax_df.loc[repeat_sax_df['seriesnumber'] == np.max(series_numbers)]
                else:
                ...
                new_sax_df_list.append(single_sax_df)
    ...
    def blob_image(self,image_4D,blobber_num = blobber_num):
        ...
        for pos in range(position): # take the quarter 
            ...
            im = tf.image.resize_with_crop_or_pad(im, cropper_image_size, cropper_image_size) # pad image to make sure it can go in the model without stretching it
            im = np.array(im)
            im = normalize(im[...,0])
        ...
        try:
            plt.imshow(images_to_crop[mid_slice_idx], cmap = 'gray')
            ...
            if np.argmin(scale_shape) == 1 and x_min < (cropper_image_size - np.min(scale_shape) )/2: # means x is overflowing
                ...
                y_min, y_max = round(y_min+ ((cropper_image_size - np.min(scale_shape) )/2 - y_min)), round(y_max + ((cropper_image_size - np.min(scale_shape) )/2 - y_min))
    ...
    def seg_image(self,image_4D):
        ...
        for time in range(timesteps):
            for pos in range(position):
                segged_image[...,pos,time,:] = color_mask_image(images[...,pos,time,:],masks[...,pos,time,:])/255

        masks = reverse_through_timesteps(masks, reverse_resize, old_shape = self.cropped_image.shape[:2]) # turns mask from 128,128 to 400,400
        masks = reverse_through_timesteps(masks, reverse_crop, crop_box = self.crop_box, original_shape = (cropper_image_size,cropper_image_size,self.image.shape[2])) # turns image back into original shape
        masks = reverse_through_timesteps(masks,reverse_pad, original_shape =  self.image.shape, scale_shape = self.scale_shape) # removes padding
        masks = reverse_through_timesteps(masks, reverse_resize, old_shape = self.image.shape[:2]) # turns image back into original shape
    ...
    def get_metrics(self, mask_4D):
        ...
        if min_heart_idx < 0:
            ...
            min_heart_idx = 0
        sum_masks = np.sum(np.sum(np.sum(np.sum(mask_4D[...,-2:],0),0),-1)[min_heart_idx:max_heart_idx,:],0) # take the 5 largest slices
