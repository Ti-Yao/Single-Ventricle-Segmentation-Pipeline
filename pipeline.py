...
class Pipeline:
    ...
    def calc_N_timesteps(self,sax_df):
        ...
        N_timesteps = stats.mode(possible_N_timesteps)[0][0]
        return N_timesteps
    ...
    def remove_repeated_scans(self,sax_df):
        ...
        sax_df = sax_df.drop_duplicates(subset = 'uid')
        for uni_slice in unique_slices:
            if len(sax_df.loc[sax_df['slicelocation'] == uni_slice]) > self.N_timesteps:
        ...
        for rep_slice in repeated_slices: 
            ...
            try:
                series_number = repeat_sax_df.loc[repeat_sax_df['uid'].isin(self.matching_uids)].seriesnumber.unique()[0]
                single_sax_df = repeat_sax_df.loc[repeat_sax_df['seriesnumber'] ==series_number]
            except:
                try:
                    ...
                    single_sax_df = repeat_sax_df.iloc[::int(len(repeat_sax_df)/self.N_timesteps), :]
